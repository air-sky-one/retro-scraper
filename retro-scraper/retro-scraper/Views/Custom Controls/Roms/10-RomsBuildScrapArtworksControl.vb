Public Class _10_RomsBuildScrapArtworksControl
    ...
    Private Sub _10_RomsBuildScrapArtworksControl_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            ...
            For Each artType As RomsDataSet.ArtworksRow In Me._parent.RomsArtworks
                Dim ckArtwork As New CheckBox
                ckArtwork.Font = New Font("Ubuntu Light", 8.25!, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, CType(0, Byte))
                ckArtwork.AutoSize = True
                ckArtwork.Name = artType.Name & "CheckBox"
                ckArtwork.Text = artType.Name
                ckArtwork.Tag = artType

                Me.CheckboxFlowLayoutPanel.Controls.Add(ckArtwork)
            Next

            Me.ArtworkDataGridView.DataSource = Me._parent.RomsArtworks
    ...
    ''' <summary>
    ''' Reset All the defined scrapping
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    Private Sub ButtonReset_Click(sender As Object, e As EventArgs) Handles ButtonReset.Click

        Dim query = From columns As DataColumn In Me._parent.RomsData.Columns Where columns.ColumnName.Contains("is_media_")

        For Each col As DataColumn In query
            For Each g As RomsDataSet.SSRomsRow In Me._parent.RomsData
                g(col) = False
            Next
        Next

        For Each art In Me._parent.RomsArtworks
            Stop
        Next
    End Sub

    ''' <summary>
    ''' Launch a window fo each selected artwork to scrap
    ''' </summary>
    ''' <param name="sender"></param>
    ''' <param name="e"></param>
    Private Sub ButtonStart_Click(sender As Object, e As EventArgs) Handles ButtonStart.Click
        Dim t As New _10_RomsBuildScrapArtworksGenericForm(Me._parent)
        t.ShowDialog()

        For Each ck As CheckBox In Me.CheckboxFlowLayoutPanel.Controls
...
'                titleLabel.Size = New Size(Me.Width - 150, 16)
'                '
'                'ActionPanel
'                '
...
'                descriptionLabel.Size = New Size(Me.Width, 16)
...
'                'Header panel
'                cb.Parent.Parent.Controls(0).BackColor = Color.Red
'                cb.Parent.Parent.Controls(0).Width = width

'                ' ck panel
'                cb.Parent.Controls(2).BackColor = Color.LightGray
'                cb.Parent.Controls(2).Width = width

'                CType(cb.Parent.Controls(3), DataGridView).AutoSize = True
'                ''CType(cb.Parent.Controls(3), DataGridView).Width = width
'                ' CType(cb.Parent.Controls(3), DataGridView).MaximumSize = New Size(width, 600)

'                'action panel
'                cb.Parent.BackColor = Color.AliceBlue
'                cb.Parent.Width = width

'                ' Main panel
'                cb.Parent.Parent.BackColor = Color.DarkGreen
'                cb.Parent.Parent.Width = width

'                cb.Parent.Parent.Controls(0).Controls(1).Width = width
'                CType(cb.Parent.Parent.Controls(1).Controls(0), Label).Width = width

'                cb.Parent.Controls(3).Visible = True 'datagrid
'               ' CType(cb.Parent.Controls(3), DataGridView).AutoSize = True
'                'CType(cb.Parent.Controls(3), DataGridView).Height = CType(cb.Parent.Controls(3), DataGridView).RowCount * 16
